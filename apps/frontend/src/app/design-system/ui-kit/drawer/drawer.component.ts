...
@Component({
  selector: 'edo-dogx-drawer',
  host: {
    '(document:keydown)': 'onDocumentKeydown($event)',
  },
  template: `
    @if (open()) {
      <div class="edo-ui-kit-drawer-backdrop" (click)="onCloseRequested()"></div>
      <section
        #drawerPanel
      ...
      >
        <header>
          <h3>{{ title() }}</h3>
          <button type="button" aria-label="Закрыть панель" (click)="onCloseRequested()">Закрыть</button>
        </header>
        <div class="edo-ui-kit-drawer__body">
          <ng-content />
        </div>
      </section>
    }
